/*
 * Copyright 2012-2015 Institute of Computer Science, Foundation for Research and Technology -
 * Hellas
 *
 ...
 * the Licence.
 *
 * Contact: POBox 1385, Heraklio Crete, GR-700 13 GREECE Tel:+30-2810-391632 Fax: +30-2810-391638
 * E-mail: isl@ics.forth.gr http://www.ics.forth.gr/isl
 *
 * Authors : Georgios Samaritakis, Konstantina Konsolaki.
 *
 * This file is part of the SchemaReader project.
 */
package gov.nist.healthcare.tools.hl7.v2.tcamt.lite.web.util.schemareader;
...
/**
 * SchemaFile is a class used to parse xsd schema files. It contains several methods to validate an
 * xml file, create xml templates or simply get element names given a xpath.
 *
 * @author samarita
 */
public class SchemaFile {
  ...
  public ArrayList<String> getElements() {
    ...
    try {
      Iterator<XSElementDecl> jtr = this.parser.getResult().getSchema(1).iterateElementDecls();
      while (jtr.hasNext()) {
  ...
  public String validate(String xml, String lang) {
    ...
    try {
      ...
      if (myErrorHandler.getLog().contains("Line: ")) {
        ...
        String lineNumber =
            errorMsg.substring(errorMsg.lastIndexOf("Line: "), errorMsg.lastIndexOf("Column: "))
                .replaceAll("Line:\\s*", "").trim();

        LinkedHashMap<Integer, String> xmlLines =
            getXMLLineByLine(xml, Integer.parseInt(lineNumber));
        int lineWithProblemIndex = Integer.parseInt(lineNumber);
        String lineWithProblem = xmlLines.get(lineWithProblemIndex);
        Utils utils = new Utils();
        ArrayList<String> elements = utils.findReg(
            "(?:<([^\\s]+)[^>]*/>|<([^\\s]+)[^>]*>(.*?)</\\2>)", lineWithProblem, Pattern.DOTALL);
  ...
  public String createLabelsAndLinksFile(String type, String xpath, String lang) {
    ...
    for (Element el : elements) {
      ...
      if (!el.fullPath.contains("/admin")) {
        ...
        if (el.attributes.containsKey("ics_vocabulary")
            || el.attributes.containsKey("sps_vocabulary")) {
        ...
        if (el.attributes.containsKey("sps_facet")) {
          temp = temp.append(
              "<facet username=\"ExternalReader\" themasUrl=\"http://139.91.183.97:8080/THEMAS/\" thesaurusName=\"TESTDATA\" facetId=\"\"/>\r\n");
        }
    ...
    for (Element el : elements) {
      ...
      if (el.getFullPath().equals(xpath) || el.getFullPath().startsWith(xpath + "/")) {
        ...
        if (!optionalPath.equals("") && el.getFullPath().startsWith(optionalPath)) {
          ...
          if (!mode.equals("maximum")) { // added medium mode = maximum except opotional elements at
                                         // admin part
            if (mode.equals("minimum")) {
              ...
              if (!isRoot && el.isOptional() && !el.name.equals("admin")
                  && el.getFullPath().contains("admin")) {
                optionalPath = el.getFullPath();
                continue;
              }
            }
