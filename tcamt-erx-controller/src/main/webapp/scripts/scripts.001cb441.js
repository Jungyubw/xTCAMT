...
angular.module('tcl').controller('MainCtrl', ['$scope', '$rootScope', 'i18n', '$location', 'userInfoService', '$modal', 'Restangular', '$filter', 'base64', '$http', 'Idle', 'notifications', 'IdleService','AutoSaveService','StorageService',
    function ($scope, $rootScope, i18n, $location, userInfoService, $modal, Restangular, $filter, base64, $http, Idle,notifications,IdleService,AutoSaveService,StorageService) {
        ...
        $rootScope.testDataCategorizations = ['PRESENCE', 'NOT_PRESENCE', 'VALUE',
            'VALUE_LIST', 'NOT_VALUE', 'NOT_VALUE_LIST'];
...
angular.module('tcl').controller('TestPlanCtrl', function ($document, $scope, $rootScope, $templateCache, Restangular, $http, $filter, $mdDialog, $modal, $cookies, $timeout, userInfoService, ngTreetableParams, $interval, ViewSettings, StorageService, $q, notifications, IgDocumentService, ElementUtils,AutoSaveService,$sce,Notification,PreferenceService) {
	...
	$scope.verifiedParsing = function () {
        var data = angular.fromJson({"message": $rootScope.selectedTestStep.xmlMessage});
        $http.post('api/xmlmessage/verifiedParsing', data).then(function (response) {
            $rootScope.verifiedParsingResult = angular.fromJson(response.data);
            console.log($rootScope.verifiedParsingResult);

            if($rootScope.verifiedParsingResult.result) {
            	$rootScope.readyTestData = true;
			}else{
                $rootScope.readyTestData = false;
			}
	...
	$scope.isManualTestStep = function(){
		if($rootScope.selectedTestStep.teststepType !== 'SUT_MANUAL'
			&& $rootScope.selectedTestStep.teststepType !== 'TA_MANUAL'
			&& $rootScope.selectedTestStep.teststepType !== 'MANUAL' ) return false;
		return true;
	};

	$scope.isTestDataReady = function() {
        if(!$scope.isManualTestStep() && $rootScope.readyTestData) return true;
        return false;
    };
	...
	$scope.initCodemirror = function () {
		if($scope.editor == null){
            var elm = document.getElementById("xmlMessage-textarea");

            if(elm){
                $scope.editor = CodeMirror.fromTextArea(document.getElementById("xmlMessage-textarea"), {
                    lineNumbers: true,
                    fixedGutter: true,
                    theme: "elegant",
                    readOnly: false,
                ...
                $scope.editor.setSize("100%", 900);
                $scope.editor.refresh();

                $scope.editor.on("change", function () {
                    $rootScope.selectedTestStep.xmlMessage = $scope.editor.getValue();
                });
			}
	...
	$scope.selectTestStep = function (testStep) {
		...
		if (testStep != null) {
            ...
            if($rootScope.selectedTestStep.xmlMessage && $rootScope.selectedTestStep.xmlMessage !== ''){
                $scope.verifiedParsing();
			}
	...
	$scope.initXMLMessageTab = function () {
		$scope.initCodemirror();
        if($scope.editor){
            setTimeout(function () {
                if($rootScope.selectedTestStep.xmlMessage == null){
                    $scope.editor.setValue("");
                }else {
                    $scope.editor.setValue($rootScope.selectedTestStep.xmlMessage);
                }
            }, 100);
	...
	$scope.refreshTree = function () {
		if ($scope.messageParams && $rootScope.readyTestData){
		$scope.messageParams.refresh();
	...
	$scope.messageParams = new ngTreetableParams({
		getNodes: function (parent) {
			if (parent && parent != null) {
				return parent.children;
			}else {
				if($rootScope.verifiedParsingResult) return $rootScope.verifiedParsingResult.data.children;
			}
			return [];
		},
		getTemplate: function (node) {
			if(node.type == 'ELEMENT') {
				if(!node.leafNode) return 'Element.html';
				else return 'LeafElement.html';
            } else return 'Attribute.html';
			return 'Element.html';
		}
	});
...
// Context menu 
	...
	$scope.testCaseOptions =[
		['Add New Test Step', function($itemScope) {
            ...
            if($rootScope.selectedTestPlan.testplanType === 'ISOLATED') newTestStep.teststepType = 'SUT_MANUAL';
            else if($rootScope.selectedTestPlan.testplanType === 'DATA_INSTANCE') newTestStep.teststepType = 'MANUAL';
